...
object MTree {
   ...
   */
  def fromLispyString(s: String): MTree[String] = LispyTreeParser.parse(s) match {
    case LispyTreeParser.Success(t, _) => t
    case _ => throw new IllegalArgumentException
  }
  object LispyTreeParser extends RegexParsers {
    def back = "^"
    def value: Parser[String] = "[a-z]+".r
    def node: Parser[MTree[String]] = "(" ~ value ~ rep(node) ~ ")" ^^ { case _ ~ v ~ children ~ _ => MTree(v, children) } | value ^^ { v => MTree(v) }
    def parse(input: String) = parseAll(node, input)
  }
}
